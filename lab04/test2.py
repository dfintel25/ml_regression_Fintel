...
# Implement Pipeline 2: Imputer → Polynomial Features → StandardScaler → Linear Regression
pipe2 = Pipeline([
    ('imputer', SimpleImputer(strategy='median')),
...
# Fit both pipelines
pipe1.fit(X1_train, y_train1)
pipe2.fit(X1_train, y_train1)

# Make predictions using both models
y_pred_pipe1 = pipe1.predict(X1_test)
y_pred_pipe2 = pipe2.predict(X1_test)

# Evaluation for Pipelines
evaluate_model(y_test1, y_pred_pipe1, "Pipeline 1: Imputer → StandardScaler → Linear Regression")
evaluate_model(y_test1, y_pred_pipe2, "Pipeline 2: Imputer → Polynomial Features → StandardScaler → Linear Regression")

# Visualize regression results for pipelines
plt.figure(figsize=(12, 6))
plt.subplot(1, 2, 1)
plt.scatter(y_test1, y_pred_pipe1, alpha=0.6)
plt.xlabel("Actual MPG")
plt.ylabel("Predicted MPG")
plt.title("Pipeline 1: StandardScaler + Linear Regression")

plt.subplot(1, 2, 2)
plt.scatter(y_test1, y_pred_pipe2, alpha=0.6, color='orange')
plt.xlabel("Actual MPG")
plt.ylabel("Predicted MPG")
plt.title("Pipeline 2: Polynomial Features + StandardScaler + Linear Regression")
